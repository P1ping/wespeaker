...
# These speakers are not included in the testing trial of CN-Celeb v2
SPK_EXCEPTIONS = {"id00902", "id00918", "id00926", "id00955"}


def key_to_class(sample_key):
    """
    Specific to CN-Celeb
    """
    if sample_key.startswith("test"):
        return sample_key[5:12]
    elif sample_key.startswith("enroll"):
        return sample_key[7:14]
    elif sample_key.startswith("id"):
        return sample_key[:7]
    else:
        raise ValueError(f"Invalid CN-Celeb sample key: {sample_key}")
...
def load_class_indices(embed_scp):
    ...
    classes = classes - SPK_EXCEPTIONS
...
def compute_nc(embed_scp, moving_average=False, classifier_weights=None, checkpoint_path=None, **kwargs):
    ...
    for key, embed in tqdm(kaldiio.load_scp_sequential(embed_scp)):
        spk = key_to_class(key)
        if spk not in class2id:
            continue
        class_idx = class2id[spk]
