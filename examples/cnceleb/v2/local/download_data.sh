...
# download_dir="$DATA_ROOT/datasets/CNCeleb"
download_dir="/mntcephfs/lee_dataset/speaker/cnceleb/cnceleb_raw_data"
...
if [ ! -f ${download_dir}/musan.tar.gz ]; then
  ...
  wget --no-check-certificate --tries=80 https://us.openslr.org/resources/17/musan.tar.gz -P ${download_dir}
...
if [ ! -f ${download_dir}/rirs_noises.zip ]; then
  ...
  wget --no-check-certificate --tries=80 https://us.openslr.org/resources/28/rirs_noises.zip -P ${download_dir}
...
if [ ! -f ${download_dir}/cn-celeb_v2.tar.gz ]; then
  ...
  wget --no-check-certificate --tries=80 https://us.openslr.org/resources/82/cn-celeb_v2.tar.gz -P ${download_dir}
...
if [ ! -f ${download_dir}/cn-celeb2_v2.tar.gz ]; then
  ...
  for part in a b c; do
    wget --no-check-certificate --tries=80 https://openslr.elda.org/resources/82/cn-celeb2_v2.tar.gza${part} -P ${download_dir} &
  done
  wait
